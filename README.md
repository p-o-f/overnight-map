...
- ~~add client-side rendering for when users swap between tabs~~ (Currently this is actually cheap, only gets "expensive" from a compute perspective if there are many, many users)
- ~~add a basic db to track # of unique visits~~ Added Google Analytics support to view user metrics ✅ 
- add auto-start on boot / auto-restart on crash (via systemd; currently using tmux + manual reboots if something goes wrong)
- add logging for GCP (so can view logs without having to SSH in) <-- relates to SystemD support (above)
...
Note that:
main.py is the "production" version running in GCP... while
my_app.py is used strictly for local testing before updating that version

Some samples of what this looks like are below, but it's better to just visit the website (these samples may be outdated, but the general idea is there).
